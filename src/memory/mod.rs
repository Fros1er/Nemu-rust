...
pub trait IOMap {
    // fn read(&self, paddr: &PAddr, size: MemOperationSize) -> u64;
    // fn write(&self, paddr: &PAddr, data:u64, size: MemOperationSize);
    fn data(&self) -> &[u8];
...
impl IOMapEntry {
    fn new(left: PAddr, right: PAddr, device: Rc<RefCell<dyn IOMap>>) -> Self {
        Self { left, right, device, dirty: false }
    }
...
impl Memory {
    ...
    pub fn find_iomap(&self, paddr: &PAddr) -> Option<&IOMapEntry> {
        for iomap in self.mmio.iter() {
            if iomap.left <= *paddr && *paddr <= iomap.right {
                return Some(iomap);
            }
        }
        None
    ...
    pub fn add_mmio(&mut self, left: PAddr, device: Rc<RefCell<dyn IOMap>>) {
        ...
        if Self::in_pmem(&io_map.left) && Self::in_pmem(&io_map.right) {
            panic!("MMIO region ({}, {}) overlaps with pmem", io_map.left, io_map.right)
        }
        if let Some(mmap) = self.find_iomap(&io_map.left).or(self.find_iomap(&io_map.left)) {
            panic!("MMIO region ({}, {}) overlaps with other mmio region ({} {})",
